...
app.post("/pizze", async (req, res) => {
    ...
    for (let k of ["naziv", "cijena", "sastojci", "slika"]) {
        if (!key.includes(k)) {
            return res.status(400).json("Moraju se proslijediti svi podaci");
        }
    }
    if (typeof podaci.cijena !== 'number') {
        return res.status(400).json("Cijena mora biti broj");
    }
...
app.post("/narudzba", async (req, res) => {
    ...
    for (let k of ["ime", "adresa", "telefon", "pizza_stavke"]) {
        if (!Object.keys(narudzba).includes(k)) {
            return res.status(400).json("Moraju se proslijediti svi podaci");
        }
    }
    ...
    for (let stavka of narudzba.pizza_stavke) {
        ...
        if (!pizza) {
            return res.status(400).json(`Pizza s nazivom ${stavka.naziv} ne postoji`);
        }
        let cijenaPoKomadu = pizza.cijena;
        ukupnaCijena += cijenaPoKomadu * stavka.kolicina;
    }
